...
> 本フェーズは **T-905 のみ先行実施**し、T-902 / T-903 / T-904 / T-907 は **P14 完了まで凍結**する。
> T-901 は新要求④（TOP/MID優先）と衝突するため **P14 の T-1401 へ縮小して移設**した（下記 P14 参照）。
> T-906 は運用タスクのため凍結対象外（パッチ毎に随時実施）。
...
| T-905 | プロンプト機密の対処: `scripts/prompts/*.j2`・`scripts/aliases.json` 等を追跡から除外し、マスタは private 管理へ移行。git履歴からの完全除去（filter-repo + force push）は影響が大きいためユーザー判断 | **最優先** | 公開リポジトリの HEAD に機密プロンプトが存在しない |
...
> **T-905 が最優先である理由**: T-804 でリポジトリを**公開化**した結果、`scripts/prompts/lane_v1.md.j2` /
> `bot_v1.md.j2` が公開状態にある。これは PRD §6「生成方式はユーザーへ公開しない」と
> [09_data_pipeline.md](./09_data_pipeline.md) §3.4 に**現状違反**しており、プロンプトへ本格投資する
> P13 に入る前に解消しなければ矛盾する。**T-1300 で退避する `bot_v1.md.j2` の保管先も T-905 の private マスタに依存する。**
...
| 番号 | 名称 | 状態 |
|---|---|---|
| P9 | 検索UI統合・AdSense申請・運用ギャップ解消 | **一部凍結**（T-905 先行 / T-906 随時 / T-901 は P14 へ移設） |
...
アドバイスの量が揃った時点で復活させる。BOT のデータ・UIコードは T-1300 で `docs/archive/bot/` へ退避し、
プロンプトテンプレートは T-905 の private マスタに保管する。復活の判断は T-1308。
